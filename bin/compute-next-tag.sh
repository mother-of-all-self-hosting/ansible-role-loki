...
#
# Tags look like `v<Loki version>-<release>`:
#
# - if defaults/main.yml points at a Loki version that has never been
#   released, the release counter restarts at 0 (`v3.7.7-0`)
# - otherwise the counter is incremented (`v3.7.7-1`), but only if something
...
version="$(sed -nE 's|^loki_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Loki version from $defaults_path"
	exit 1
fi

# The version values do not carry a leading `v` (e.g. `3.7.7`), but the tags
# do (`v3.7.7-0`). Stripping any `v` before prepending one keeps this correct
